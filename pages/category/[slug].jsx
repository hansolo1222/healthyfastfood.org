...
export const getServerSideProps = async (context) => {
  ...
  const meals = await prisma.meal.findMany({
    where: {
      category: {
        parentCategorySlug: String(context.params?.slug),
      }
    },
    include: {
      category: true,
...
export default function Restaurant(props) {
  ...
  const {meals, parentCategories, parent, restaurants} = props;
        ...
        <div className="flex">
          ...
          <main className="mt-8 w-full">
            <div className="flex items-center">
              {/* <div className="relative w-14 h-14 mr-4">
                <Image
                  ...
                  src={`/images/logosSmall/${category.slug}.webp`}
                  alt={`${category.name} Logo`}
                  layout="fill"
                  objectFit="contain"
                />
              </div> */}
              <div className="mr-4 text-5xl border rounded-xl p-4">
                {formatParentCategory(category.slug,false,true,false)}
              </div>
              <div>
                <Breadcrumbs pages={pages} />
                <h1 className="text-3xl font-bold mt-1">
                  Ranking {category.name} at Restaurant Chains
                </h1>
                <h2 className="text-xl font-normal mt-1 text-stone-500">
            ...
            {/* <p className="text-stone-500 mt-2 mb-2">
                Nutrition information for all menu tems from {category.name}.
